...
"""
Modified from pymoo to have inversed axis
"""
class PCP(Plot):
        ...
        for i in range(self.n_dim):
            ...
            if self.show_bounds:
                if i in self.reverse_index:
                    upper = self.ax.text(i - margin_left, bottom, self.func_number_to_text(abs(bounds[0][i])))
                    lower = self.ax.text(i - margin_left, top, self.func_number_to_text(abs(bounds[1][i])))
                else:
